  ...
  return (
    <div className='px-36 flex flex-col gap-[6rem] mt-[18rem] max-sm:mt-[10rem] justify-center max-sm:px-10 max-lg:px-10'>
      <div className="flex flex-col justify-center text-center gap-5">
        <h2 style={textStyle} className='2xl:text-6xl max-sm:text-3xl sm:text-4xl font-extrabold tracking-tight'>{text.split('').map((letter, index) => (<span key={index}>{letter}</span>))}</h2>
      ...
      <div className="grid xl:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 max-sm:grid-cols-1 max-sm:gap-10 place-items-center">
        {features.map((feature) => (
          <div key={feature.id} className="w-[15rem] flex flex-col gap-5 p-8 hover:cursor-pointer max-sm:w-[80%] hover:scale-105 hover:shadow-xl rounded-xl">
